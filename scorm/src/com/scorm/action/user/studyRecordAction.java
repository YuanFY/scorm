...
@Component(value="studyrecordAction")
public class studyRecordAction extends ActionSupport{
	/**分页
	 * 
	 */
...
//	String studyStatus;
	...
	public String execute(){
		...
		List<Studyrecordinfo> list = usercourseinfoService.getStudyRecord(userId);
		if(list!=null){
			dataCount = list.size();
		}else{
			dataCount = 0;
		...
		//		System.out.println((pageStart -1 ) * page.getPageRecordNum()+","+ ((pageStart -1 ) * page.getPageRecordNum()+page.getPageRecordNum())+":::");
				...
				//System.out.println((pageStart -1 ) * page.getPageRecordNum()+";;;"+dataCount);
				//System.out.println(usercourseinfoService.getStudyCourse(userId).get(0).getCourseName());
	...
	//	System.out.println(courseType+"::"+courseName);
		...
		{
			list = usercourseinfoService.getStudyRecord(userId);
			
		}
		else if(courseName.equals("")&&!courseType.equals("")){
			list = usercourseinfoService.getStudyRecordByCourseType(courseType, userId);
		}
		else if(!courseName.equals("")&&courseType.equals("")){
			
			list = usercourseinfoService.getStudyRecordByCourseName(courseName, userId);
		}else{
			list = usercourseinfoService.getStudyRecordByAll(courseName, courseType, userId);
		}
